...
public class PngBuilder
{
    ...
    public static PngBuilder FromBgra32Pixels(Stream data, int width, int height, bool useAlphaChannel = true)
    {
        ...
        for (var y = 0; y < height; y++)
        {
            for (var x = 0; x < width; x++)
            {
                ...
                if (useAlphaChannel)
                {
                    builder.SetPixel(new UniColor(buffer[3], buffer[2], buffer[1], buffer[0]), x, y);
                }
                else
                {
                    builder.SetPixel(buffer[2], buffer[1], buffer[0], x, y);
                }
            }
        }
    ...
    /// </summary>
    public PngBuilder SetPixel(byte r, byte g, byte b, int x, int y) => SetPixel(new UniColor(r, g, b), x, y);
    ...
    private static int PixelToColorInt(UniColor p) => PixelToColorInt(p.R, p.G, p.B, p.A);
